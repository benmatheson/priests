...
map1.on("load", function () {
    ...
    map1.on('mousemove', 'ct', function (e) {
        ...
        var priestsList = priests.filter(function (d) {
            return d.location_simple == cityName;
        });
        var priestsOnly = priestsList.filter(function (d) {
            return d.loc != "NOTES";
        });

        var notes = priestsList.filter(function (d) {
            return d.loc == "NOTES";
        });

        console.log(priestsList);
        console.log("NOTES LENG" + notes.length);
        var pop = e.features[0].properties.population;

        var popTable = "\n<p class=\"popPre\">" + (cityName == "Barrow (Utqiagvik)" ? "Utqiagvik (Barrow)" : cityName) + "</p>\n<p class=\"popPop\">Population: " + pop.toLocaleString() + "</p>\n\n" + priestsOnly.map(function (d, i) {
            return "<p class=\"popName\">" + d.priests + "</p>";
